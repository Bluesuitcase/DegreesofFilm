...
| Name | Value | Defined in | What it does | Gate |
|---|---|---|---|---|
| Routes | `?` home · `?modes` · `?play` daily · `?play&mode=poser` · `?play&mode=buff` (Movie Buff, added 2026-07-11: cinephile rules + autocomplete on every rung — titles from `title-index.json`, people from `people-index.json` (credits-harvested, rebuild occasionally via `curation/people_index.py --source credits`); both fetched only in this mode; no daily-stats writes) · `?id=N` archive replay · `?archive` index · `?history` score history (added 2026-07-11: this device's Cinephile daily results from `stats.history`) · `?connect[=N]` graph-mode Connect prototype (added 2026-07-13: loads docs/challenges/NNN.json, chain.js engine; quiet route, no tile until G4) · `?practice` chooser · `?practice&mode=cinephile\|poser` endless | docs/app.js `init()` | view selection; unknown `mode` values fall back to `cinephile` | change-control (player-facing) |
...
| Worker URL | `https://dof-match.bluesuitcase.workers.dev` (deployed 2026-07-11, GATE 1 passed, live in `MATCH_API` since 07-11) | Cloudflare (worker `dof-match`) | the /match endpoint the client calls in production | change-control (player-facing) |
...
| `MATCH_API` (v3 Phase 1; **ON since 2026-07-11**, PR #21) | `'https://dof-match.bluesuitcase.workers.dev'` (`''` = OFF, the one-line rollback) | docs/app.js | guesses are verified by `POST /match` (cinephile mode only — poser's trimmed ladder re-indexes rungs); any failure falls back to local matching within 2 s | change-control (player-facing) + server-move-campaign |
